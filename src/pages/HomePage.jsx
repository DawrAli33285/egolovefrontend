    ...
    <div className="min-h-screen bg-gradient-to-br from-[#1E1B4B] via-[#312E81] to-[#5B21B6] text-white relative overflow-hidden">
      ...
      <div className="max-w-xl mx-auto px-5 relative z-10">
        ...
        <nav className="flex justify-between items-center py-5">
         ...
         {user?<Link to="/profile" className="bg-white/10 border border-white/20 text-white px-5 py-2 rounded-full text-sm font-semibold hover:bg-white/20 transition">
              {t('nav.profile')}
            </Link>:<Link to="/login" className="bg-white/10 border border-white/20 text-white px-5 py-2 rounded-full text-sm font-semibold hover:bg-white/20 transition">
              {t('nav.login')}
            </Link>}
          </div>
        </nav>
        ...
        <div className="text-center pt-8 pb-6">
          <div className="flex justify-center mb-5">
            <img src={logoImg} alt="EgoXLove" className="md:w-[300px] w-[50%] " />
          </div>
          ...
          <p className="text-lg opacity-85 max-w-md mx-auto mb-8 leading-relaxed">
            {t('home.subline')}
          </p>
         {user?<Link
            to={user?.isPremium?'/premium-quiz':'/quiz'}
        ...
        <div className="flex flex-wrap justify-center gap-2 py-4">
          {PILLAR_PILLS.map(p => (
            <span key={p} className="bg-white/7 border border-white/10 px-3 py-1.5 rounded-full text-[11px] whitespace-nowrap">
              {p}
            </span>
          ))}
        </div>
        ...
        <div className="text-center pb-12">
          ...
          <Link to="/register" className="text-[#C4B5FD] font-semibold text-sm underline underline-offset-2">
            {t('nav.register')}
          </Link>
        </div>
      </div>
