    ...
    return (
      <div className="min-h-screen bg-gradient-to-b from-violet-50 to-gray-50 flex items-center justify-center p-5">
        <div className="text-center max-w-sm">
          <div className="w-20 h-20 rounded-full bg-gradient-to-br from-yellow-400 to-yellow-600 flex items-center justify-center text-4xl mx-auto mb-6 shadow-2xl shadow-yellow-200">
            ⭐
          </div>
          <h2 className="text-2xl font-black text-indigo-950 mb-2">Welcome to Premium!</h2>
          <p className="text-gray-500 text-sm mb-8 leading-relaxed">
            Your plan is now active. Dive into the full 198-question quiz and unlock your detailed report.
          </p>
          <div className="flex flex-col gap-3">
            <button
              onClick={() => navigate('/premium-quiz')}
              className="w-full bg-gradient-to-r from-yellow-500 to-yellow-700 text-white py-4 rounded-2xl font-black text-sm hover:brightness-110 transition shadow-lg"
            >
              ⭐ Start Premium Quiz →
            </button>
            <Link to="/profile" className="text-sm text-violet-600 font-semibold hover:underline">
              Back to Profile
            </Link>
          </div>
        </div>
  ...
  {PLANS.map(p => {
    ...
    return (
      ...
      >
                ...
                {isActive ? (
                  <div className="w-full py-3 rounded-xl text-center text-xs font-black text-violet-600 border-2 border-violet-200 bg-violet-50">
                    ✓ Current Plan
                  </div>
                ) : (
                  <button
        ...
        <div className="grid grid-cols-3 gap-3 mb-8 text-center">
          {TRUST.map(t => (
            <div key={t.label} className="bg-white rounded-2xl p-4 shadow-sm border border-gray-100">
              <div className="text-2xl mb-1">{t.icon}</div>
              <div className="text-xs font-bold text-indigo-950">{t.label}</div>
        ...
        <div className="text-center">
          <Link to="/profile" className="text-sm text-violet-600 font-semibold hover:underline">← Back to Profile</Link>
        </div>
