...
//     mid: { label: 'Growing', color: 'border-amber-400', badge: 'bg-amber-100 text-amber-700',
//       quote: '"Vulnerability is the birthplace of love and belonging." — Brené Brown',
...
//     mid: { label: 'Growing', color: 'border-amber-400', badge: 'bg-amber-100 text-amber-700',
//       quote: '"Ho\'oponopono: I love you, I\'m sorry, please forgive me, thank you."',
//       rec: 'Practice the Ho\'oponopono technique daily for 5 minutes with a difficult relationship.' },
//     low: { label: 'To develop', color: 'border-yellow-600', badge: 'bg-yellow-100 text-yellow-800',
...
// function getPillarRec(loveScore, recs) {
...
//   { freq: 'Weekly', action: 'Ho\'oponopono practice for one difficult relationship (5 min)' },
...
//   { freq: 'Diario', action: 'Gratitud matutina (3 cosas) + 5 min de respiración consciente' },
//   { freq: 'Diario', action: 'Revisión nocturna: "¿Mis acciones estuvieron alineadas con mis valores hoy?"' },
//   { freq: 'Semanal', action: 'Un acto de bondad aleatorio sin expectativa de retorno' },
//   { freq: 'Semanal', action: 'Práctica de Ho\'oponopono para una relación difícil (5 min)' },
//   { freq: 'Mensual', action: 'Revisión profunda de tu pilar prioritario con journaling' },
...
//     setPdfLoading(true);
//     setShowPopup(false);
//     setTimeout(() => { window.print(); setPdfLoading(false); }, 300);
//   };
...
//           <h3 className="text-xl font-black text-indigo-950 mb-2">Premium Report Sent!</h3>
//           <p className="text-sm text-gray-500 leading-relaxed mb-6">Your detailed premium report has been sent to your email. Check your inbox!</p>

//           <div className="border-t border-gray-100 mb-6" />

//           <div className="w-16 h-16 rounded-full bg-violet-100 flex items-center justify-center text-3xl mx-auto mb-4">📥</div>
//           <h3 className="text-xl font-black text-indigo-950 mb-2">Download PDF</h3>
//           <p className="text-sm text-gray-500 leading-relaxed mb-6">Save your full premium report as a PDF to keep or share.</p>

//           <button
//             onClick={handleDownloadPDF}
//             disabled={pdfLoading}
//             className="w-full bg-gradient-to-r from-violet-600 to-purple-700 text-white py-4 rounded-2xl font-black text-sm hover:brightness-110 hover:-translate-y-0.5 transition-all duration-200 shadow-lg shadow-purple-200 disabled:opacity-60 mb-3"
//           >
//             {pdfLoading ? 'Preparing…' : '📥 Download Premium Report as PDF'}
//           </button>
//           <button onClick={() => setShowPopup(false)} className="w-full py-3 text-sm text-gray-400 hover:text-gray-600 transition font-medium">
...
//                 </span>
//                 <span className="border border-yellow-600 text-yellow-700 text-[9px] font-black px-2 py-0.5 rounded-full">
//                   RAPPORT PREMIUM ⭐
//                 </span>
//               </div>
...
//             <button
//               onClick={handleDownloadPDF}
//               className="no-print flex items-center gap-1.5 bg-indigo-950 text-white px-5 py-2.5 rounded-xl text-xs font-bold hover:bg-indigo-800 transition"
//             >
//               📥 Download PDF
//             </button>
//           </div>
...
//               const pillarT = t(`pillars.${p.id}`, { returnObjects: true });
//               return (
...
//                   <div className="w-7 h-7 rounded-full bg-violet-600 text-white flex items-center justify-center text-xs font-black flex-shrink-0">
//                     {idx + 1}
//                   </div>
//                   <div className="flex-1">
...
//             className="text-xs text-violet-600 font-semibold hover:underline block mx-auto"
//           >
//             ← Retake free quiz
//           </button>
...
// ─────────────────────────────────────────────────────────────────────────────
// Pure-data PDF builder using jsPDF draw calls — NO html2canvas / screenshot
// ─────────────────────────────────────────────────────────────────────────────
function buildDataPdf(global, pillarPercents, pillars, actionPlan, lang) {
  ...
  const setFill  = (hex) => { const [r,g,b] = hexToRgb(hex); pdf.setFillColor(r,g,b); };
  const setDraw  = (hex) => { const [r,g,b] = hexToRgb(hex); pdf.setDrawColor(r,g,b); };

  const wrappedText = (text, x, maxW, lineH) => {
    const lines = pdf.splitTextToSize(String(text), maxW);
    lines.forEach(l => { newPageIfNeeded(lineH + 2); pdf.text(l, x, y); y += lineH; });
    return lines.length;
  };
  ...
  sorted.slice(0, 3).forEach((p, idx) => {
    ...
    setFill('#7c3aed');
    pdf.circle(MARGIN + 3.5, y + 2, 3.5, 'F');
    ...
    setDraw('#f3f4f6');
    pdf.line(MARGIN, y + 7, MARGIN + COL, y + 7);
  ...
  const freqColors = { Daily: '#6d28d9', Diario: '#6d28d9', Weekly: '#7c3aed', Semanal: '#7c3aed', Monthly: '#a78bfa', Mensual: '#a78bfa' };
  ...
  actionPlan.forEach(item => {
    ...
    aLines.forEach((l, i) => { pdf.text(l, MARGIN + 22, y + i * 4); });
...
export default function PremiumReportPage() {
  ...
  // Build pillar list with translated names for the PDF
  const getPillarsForPdf = () => {
    return PILLARS.map(p => {
      const pillarT = t(`pillars.${p.id}`, { returnObjects: true });
      return { ...p, name: pillarT?.name || p.name };
    });
  };
  ...
  const handleSendReport = async () => {
    ...
    try {
      ...
      // Build the PDF programmatically — no screenshot, no huge blob
      const pdf = buildDataPdf(global, pillarPercents, getPillarsForPdf(), actionPlan, lang);
  ...
  return (
    <>
      ...
      {showPopup && (
        <div className="no-print fixed inset-0 z-[999] flex items-center justify-center p-5">
          <div className="absolute inset-0 bg-black/50 backdrop-blur-sm" onClick={() => setShowPopup(false)} />
          <div className="relative bg-white rounded-3xl shadow-2xl w-full max-w-sm p-8 text-center animate-[fadeInUp_0.3s_ease] max-h-[90vh] overflow-y-auto">
            <button
            ...
            <p className="text-sm text-gray-500 leading-relaxed mb-5">
              Enter your email to receive your full premium results report.
            </p>
            ...
            <label className="flex items-start gap-3 text-left mb-6 cursor-pointer">
              ...
              <span className="text-xs text-gray-500 leading-relaxed">
                I wish to receive the report by email and agree to be contacted exclusively by this service.
              </span>
            </label>
      ...
      <div className="min-h-screen bg-gray-50">
            ...
            <div className="flex justify-between items-start flex-wrap gap-3">
              ...
              <div className="no-print flex gap-2 flex-wrap">
                <button
                  ...
                  className="flex items-center gap-1.5 bg-violet-600 text-white px-5 py-2.5 rounded-xl text-xs font-bold hover:bg-violet-700 transition"
                >
                  📧 Send by Email
                </button>
                <button
                  onClick={handleDownloadPDF}
                  disabled={pdfLoading}
                  className="flex items-center gap-1.5 bg-indigo-950 text-white px-5 py-2.5 rounded-xl text-xs font-bold hover:bg-indigo-800 transition disabled:opacity-60"
                >
        ...
        {/* ── Report body (on-screen view — unchanged) ── */}
        <div ref={reportRef} className="max-w-2xl mx-auto px-5 py-6 space-y-6">
          ...
          <div className="text-center space-y-2">
            <button onClick={() => navigate('/quiz')} className="text-xs text-violet-600 font-semibold hover:underline block mx-auto">
              ← Retake free quiz
            </button>
            <Link to="/" className="text-xs text-gray-400 hover:text-gray-600 transition block">
              Back to home
