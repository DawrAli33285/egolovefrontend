...
const LIKERT = [
  { value: 0,   labelEn: "Not at all",   labelEs: "Para nada",       emoji: "😶" },
  { value: 2.5, labelEn: "A little",     labelEs: "Un poco",         emoji: "🤏" },
  { value: 5,   labelEn: "Moderately",   labelEs: "Moderadamente",   emoji: "🙂" },
  { value: 7.5, labelEn: "A lot",        labelEs: "Bastante",        emoji: "😊" },
  { value: 10,  labelEn: "Totally",      labelEs: "Totalmente",      emoji: "🔥" },
];


const SECTION_LABEL = {
  "---": { en: "Core",               es: "Esencial",          icon: "⭐" },
  A:    { en: "Origins",             es: "Orígenes",          icon: "🌱" },
  B:    { en: "Reactions",           es: "Reacciones",        icon: "⚡" },
  C:    { en: "Capacity to Change",  es: "Capacidad de Cambio", icon: "🔄" },
  D:    { en: "Life Impacts",        es: "Impactos",          icon: "🌍" },
};
...
// function computePremiumResults(answers) {
//   const pillarData = {};

//   PREMIUM_QUESTIONS.forEach((q, i) => {
//     const answerIndex = answers[i]; 
//     if (answerIndex === undefined) return;
//     const score = q.scores[answerIndex]; 
...
function PillarDots({ current, total, answers }) {
...
export default function PremiumQuizPage() {
  ...
  const total = PREMIUM_QUESTIONS.length; 
  const perPillar = Math.ceil(total / 9); 
  ...
  const q        = PREMIUM_QUESTIONS[current];
  const col      = PILLAR_COLORS[q.pillar - 1];
  ...
  const handleSelect = (likertIndex) => {
  ...
  const finishQuiz = () => {
    const results = computePremiumResults(answers);
  ...
  const questionText = lang === "es" ? q.es : q.en;
  const typeLabel    = q.type === "EGO"
    ? (lang === "es" ? "Afirmación EGO" : "EGO Statement")
    : (lang === "es" ? "Afirmación LOVE" : "LOVE Statement");
          ...
          <div className="flex justify-between items-center">
            <div className="flex items-center gap-2">
              <Link to="/" className="shrink-0">
                <img src={logoImg} alt="EgoXLove" className="w-10 h-10 object-contain" />
              </Link>
    ...
    DEV: Fill All
      ...
      <div className="flex-1 max-w-xl mx-auto w-full px-4 py-5 flex flex-col gap-4">
        ...
        {answeredCount === total && (
          ...
          >
            {lang === "es" ? "Ver resultados →" : "Go to results →"}
          </button>
        )}
      </div>
    </div>
